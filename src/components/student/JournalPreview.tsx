...
const JournalPreview = () => {
    return (
        <Card className="p-5 h-full flex flex-col rounded-lg">
            ...
            <div className="space-y-2 flex-grow overflow-hidden">
                {demoJournalTop3.slice(0, 3).map(entry => (
                    <Link to={`/student/journal?entry=${entry.id}`} key={entry.id} className="block p-2 -mx-2 group hover:bg-muted/50 rounded-lg transition-colors">
